        ...
        with open(antodir + fl) as fd:
            ...
            for l in fd:
                ...
                if x.startswith("Reg:"):
                    ...
                    (x.startswith("Status:") or x.startswith("New Status:"))
                    and "Valid Registration" in x
                    and reg is not None
                    and icao_type is not None
                    and hex_code is not None
                ):
                    existing = (
                        session.query(Registration)
                        .filter_by(icao_code=hex_code)
                        .first()
                    )
                    if existing is not None:
                        ...
                        session.query(Registration).filter_by(
                            ...
                            {"registration": reg, "equip": icao_type},
                            synchronize_session="evaluate",
                        )
                    else:
                        pyradar.logger.info(
                            "Add icao: {} reg: {}".format(hex_code, reg)
                        )
