...
with open("config.toml", "rb") as f:
    ...
    s_types_to_write = config["output"]["types_to_write"]
    ...
    fname = "{}/{}".format(
        config["directories"]["log"], config["logging"]["socketradar"]
    )
    fh = logging.handlers.TimedRotatingFileHandler(
        fname, when="midnight", interval=1, backupCount=5
    )
    ...
    while True:
        ...
        post_to_slack(
            "socketradar {0}connected on {1} ({2})".format(repeat, os.uname()[1], myip)
        )
        ...
        while True:
            ...
            try:
                ...
                if len(buf) < 1:
                    ...
                    break
                logger.info(
                    "{2}: Writing {0} bytes to {1}".format(
                        str(len(buf)), o_file_base, str(datetime.now())[:19]
                    )
                )
                tm_day_mins = (
                    ...
                    if len(parts) > 4:
                        ...
                        if icao != "000000" and is_valid_icao(icao):
                            if icao not in seen_planes:
                                ...
                                recently_seen[icao] = reg
                                post_to_slack(
                                    repeat_msg_url.format(
                                        icao=icao, reg=reg, equip=equip
                                    )
                                )
                        else:
                            logger.error(
                                "{0}: invalid ICAO code <{1}>".format(
                                    str(datetime.now())[:19], icao
                                )
                            )
