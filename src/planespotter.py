...
class SpottedPlane(Plane):
    ...
    def can_slack(self):
        return (self.slacked['track'] is False and self.has_track()) or (self.slacked['alt'] is False and self.has_alt() and self.slacked['gs'] is False and self.has_gs()) or (self.slacked['nearest'] is False and self.has_nearest())
...
config.read('planespotter.props')
config.read('planespotter.local.props')
...
fname = '{}/{}'.format(config.get('directories', 'log'), config.get('logging','planespotter'))
...
while True:
    ...
    post_to_slack('planespotter {0}connected on {1} ({2})'.format(repeat, os.uname()[1], myip))
                ...
                if parts[0] in ('MSG', 'MLAT') and parts[4] != '000000':
                    ...
                    if is_valid_icao(icao):
                        if icao not in seen_planes:
                            ...
                            if plane.can_slack():
                                post_to_slack('seen plane: {0}'.format(plane.slack_msg()))
                        else:
